...
Replicator.prototype = {
	...
	index: function() {
		return this._getJson(this.options.target + "/_design/app/_rewrite");
	},
	...
	_replicateWithAttachments: function(doc) {
		...
		.then(function(){
			...
			var putUrl = self.options.target + "/" + encodeURIComponent(doc.id)
	...
	_processJson: function(json) {
		...
		_.each(json.versions, function(vJson){
			...
			if (!vJson.dist.length) {
				vJson.dist.length = json._attachments[base] ? json._attachments[base].length : 0;
			}
